const showDetails = async () => {
	try {
		...
		const boxDetail = document.querySelector(".boxDetail") as HTMLDivElement;
		const url = new URLSearchParams(window.location.search);
		const urlId = url.get("productId");
		const dataDetail = data.find((item: any) => item.id === urlId);
		if (dataDetail) {
		...
		const sendCommentBtn = document.getElementById("sendComment") as HTMLButtonElement;
		if (sendCommentBtn !== null) {
			sendCommentBtn.addEventListener("click", async () => {
				const commentsInput = (
					document.getElementById("inforComment") as HTMLTextAreaElement
				).value;
		...
		fetch("http://localhost:5000/products")
			.then((res) => res.json())
			.then((data) => {
				const commentList = data.find((item: any) => item.id === idOfProduct);
				const boxComments = document.getElementById("listComments") as HTMLDivElement;
				if (boxComments !== null) {
					const arrComments = commentList.comments;
					arrComments.forEach(({ username, content }: any) => {
